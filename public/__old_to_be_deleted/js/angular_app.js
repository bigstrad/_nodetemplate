...
app.config(function ($routeProvider,
	...
	$routeProvider
        ...
        .when('/view/404', {
            templateUrl: '/partials/404.html', controller: '404Ctrl'
        })
		.when('/view/login', {
		...
		.when('/view/orders', {
			templateUrl: '/partials/order-list.html', controller: 'OrderListCtrl'
		})
		.when('/view/ordersave', {
			templateUrl: '/partials/order-save.html', controller: 'OrderSaveCtrl'
		})
		/*.when('/orders/:order_id', {
			templateUrl: '/partials/order-detail.html', controller: 'OrderDetailCtrl'
		})*/
		.otherwise({
//			redirectTo: '/'
            redirectTo: '/view/404'
			...
			return promise.then(function(successResponse) {
				switch (successResponse.status) {
					case 200:
						...
						break;
					case 401:
						showMessage('401 Wrong username or password', 'alert-danger', 20000);
						return $q.reject(successResponse);
						break;
					case 403:
						showMessage('403 You don\'t have the appropriate permission', 'alert-danger', 20000);
						return $q.reject(successResponse);
						break;
					case 500:
						showMessage('500 Internal server error', 'alert-danger', 20000);
						return $q.reject(successResponse);
						break;
					default:
						if (successResponse.config.method.toUpperCase() == 'GET'){
							showMessage('Error ' + successResponse.status + ': ' + successResponse.data, 'alert-danger', 20000);
							return $q.reject(successResponse);
						} else {
							showMessage('Success', 'alert-success', 5000);
							return successResponse;	
				...
				switch (errorResponse.status) {
					...
					case 500:
						...
						break;
					default:
						showMessage('Error: ' + errorResponse.status + ': ' + errorResponse.data, 'alert-danger', 20000);
				}
